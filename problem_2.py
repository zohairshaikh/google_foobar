...
def bubble_sort(ver_arr):
    for i in range(ver_arr.__len__() - 1):
        switch = False
        try:
            if int(ver_arr[i][0]) > int(ver_arr[i + 1][0]):
                switch = True
                continue

            elif int(ver_arr[i][0]) == int(ver_arr[i + 1][0]):

                if len(ver_arr[i]) > 1:
                    if len(ver_arr[i + 1]) > 1:
                        if int(ver_arr[i][1]) > int(ver_arr[i + 1][1]):
                            switch = True
                            continue
                        elif int(ver_arr[i][1]) == int(ver_arr[i + 1][1]):
                            if len(ver_arr[i]) > 2:
                                if len(ver_arr[i + 1]) > 2:
                                    if int(ver_arr[i][2]) > int(ver_arr[i + 1][2]):
                                        switch = True
                                else:
                                    switch = True
                        else:
            ...
            if switch:
                temp = ver_arr[i]
                ver_arr[i] = ver_arr[i + 1]
                ver_arr[i + 1] = temp
    return ver_arr


def rejoin_version(lis):
    output = []
    for num in lis:
        temp = ".".join(num)
        output.append(temp)
...
def solution(l):
    ver_arr = []
    for ver in l:
        ver_arr.append(ver.split('.'))
    for x in range(len(ver_arr)):
        ver_arr = bubble_sort(ver_arr)

    print ",".join(rejoin_version(ver_arr))

...
def getParentForIndex(height, index):
    import math
    if height < 0:
        return -1
    max_index = int(math.pow(2, height) - 1)
    if index >= max_index:
        return -1
    half = int(max_index / 2)
    if index == max_index - 1 or index == (max_index - 1 - max_index / 2):
        return max_index
    else:
        if index > half:
            return half + getParentForIndex(height - 1, index - half)
        else:
            return getParentForIndex(height - 1, index)


def solution(height, indexes):
    output = []
    for i in indexes:
        output.append(getParentForIndex(height, i))
